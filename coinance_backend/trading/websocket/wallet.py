...
def broadcast_ws_wallet_updated(wallet: Wallet):
    message_type = 'wallet_updated'
    serialized = WalletSerializer(instance=wallet)
    message = {
        'type': message_type,
    ...
    group_name = to_ws_group_name(wallet.user.pk)
...
class WalletConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def receive_json(self, content, **kwargs):
        ...
        if message_type == 'subscription':
            ...
            await self.subscribe_wallet(access_token)

        elif message_type == 'unsubscription':
            user_id = int(content['user_id'])
            await self.unsubscribe_wallet(user_id)
    ...
    async def subscribe_wallet(self, access_token: str):
    ...
    async def unsubscribe_wallet(self, user_id: int):
        group_name = to_ws_group_name(user_id)

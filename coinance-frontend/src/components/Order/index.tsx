...
const Order: React.FC<OrderProps> = (props) => {
  ...
  const commission = isBuy
    ? 0
    : total * (props.currencyPair?.commission_rate || 0);
    ...
    <Card elevation={0}>
      <CardContent>
        ...
        <Typography variant="body2">
          {isBuy
            ? `매수가능 ${currencyFromSymbol}`
            : `매도가능 ${currencyToSymbol}`}
          : <b>{f(props.wallet?.available_amount || 0).toFixed(8)}</b>
        </Typography>
      </CardContent>
      <Divider />
      ...
      <CardContent>
        ...
        <TextField
          ...
          onChange={(e) =>
            (props.control.form.price = f(Number(e.target.value)))
          }
        ...
        <TextField
          ...
          InputProps={{
            endAdornment: (
              <InputAdornment position="end">{currencyToSymbol}</InputAdornment>
            ),
          }}
          value={props.control.form.amount}
          onChange={(e) =>
            (props.control.form.amount = f(Number(e.target.value)))
          }
